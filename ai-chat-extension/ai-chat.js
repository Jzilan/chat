...
function persist(k)  { if (!k || k === 'chars')   LS.set('chars', S.chars); if (!k || k === 'active')  LS.set('active', S.active); if (!k || k === 'msgs')    LS.set('msgs', S.msgs); if (!k || k === 'api')     LS.set('api', S.api); }
...
@keyframes aims{from{transform:scale(.95) translateY(10px);opacity:0}to{transform:scale(1) translateY(0);opacity:1}}
...
// ───────── 人物面板 ─────────
function charsPanel() {
  const p = el('div', { id: 'aic-pl-characters', c: 'aic-ip', s: { display: 'none', flexDirection: 'column', height: '100%' } });
  p.innerHTML = `
...
function settingsPanel() {
  const p = el('div', { id: 'aic-pl-settings', c: 'aic-ip', s: { display: 'none', flexDirection: 'column', height: '100%' } });
  p.innerHTML = `
...
function showTab(id) {
  ['chat','characters','settings'].forEach(k => { const e = document.getElementById('aic-pl-' + k); if (e) e.style.display = k === id ? 'flex' : 'none'; });
}
...
function openPanel() {
  ...
  const ball = document.getElementById('aic-b');
